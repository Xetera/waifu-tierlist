...
router.get(
  ...
  sendAnime(
    async (id: string): Promise<CharacterSearchResponse> => {
...
router.get(
  ...
  sendSave(
    async (url): Promise<SaveLookupResponse> => {
      try {
        ...
        if (!anime) {
          return Promise.reject({
            error: `Anime recorded as ${animeId} on a save could not be found in the database`
          });
        }
